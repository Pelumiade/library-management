...
def borrow_book(
    ...
    book = books.book.get(db, id=lending_in.book_id)
    if not book:
   ...
   # Check if user exists
    user = users.user.get(db, id=lending_in.user_id)
    if not user:
    ...
    lending_data = {column.name: getattr(lending_record, column.name) for column in lending_record.__table__.columns}
...
def return_book(lending_id: int, db: Session = Depends(get_db)):
   ...
   # Find the lending record
    lending_record = db.query(models.Lending).filter(models.Lending.id == lending_id).first()
    ...
    try:
        ...
        # Convert lending object to dictionary for event publishing
        lending_data = {column.name: getattr(returned_lending, column.name) for column in returned_lending.__table__.columns}
